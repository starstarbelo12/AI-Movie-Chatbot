...
def detect_movie_specific_attributes(user_message, movie_titles):
        ...
        for i, (start, end, title) in enumerate(
            title_positions
        ):

            # Text immediately before this movie,
            # starting after the previous movie.
            if i == 0:
                before_start = 0
            else:
                before_start = title_positions[i - 1][1]

            before_segment = normalized_message[
                before_start:end
            ].strip()

            # Text immediately after this movie,
            # stopping before the next movie.
            if i + 1 < len(title_positions):
                after_end = title_positions[i + 1][0]
            else:
                after_end = len(normalized_message)

            after_segment = normalized_message[
                end:after_end
            ].strip()

            before_attributes = detect_requested_attributes(
                before_segment
            )

            after_attributes = detect_requested_attributes(
                after_segment
            )

            combined = []

            for tag in before_attributes:
                if tag not in combined:
                    combined.append(tag)

            for tag in after_attributes:
                if tag not in combined:
                    combined.append(tag)

            if combined:
                result[title] = combined
...
def chatbot_response(user_message, algorithm="hybrid"):
    ...
    movie_specific_attributes = (
    detect_movie_specific_attributes(
        corrected_message,
        comparison_titles,
    )
)
    ...
    # Movie-specific multi-attribute queries take priority
    # over generic comparison.
    if movie_specific_attributes:
        forced_tag = None
    ...
    if movie_specific_attributes:
        ...
        for movie_title, requested_tags in (
            movie_specific_attributes.items()
        ):
            row = get_movie_row(movie_title)
            ...
            for requested_tag in requested_tags:
                formatted = format_intent_response(
                    requested_tag,
                    row,
                )
